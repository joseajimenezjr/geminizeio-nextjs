...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (isPreviewMode) {
    console.log(`API: Preview mode detected, simulating toggle of accessory ${accessoryID} to ${active}`)
    return NextResponse.json({
      success: true,
      message: `Accessory ${accessoryID} toggled to ${active ? "on" : "off"} in preview mode`,
    })
  }
